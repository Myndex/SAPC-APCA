...
var SAPCparams = {
  ...
  blkClmpInvert: 1.51484594658864, // for inversion of 1.414
  blkClmpInvFactor: 2.06721614848681, // for inversion only
...
function SAPCcontrast(Ytxt, Ybg, places = -1) {
  ...
  return places == -1
    ? outputContrast * 100.0
    : outputContrast != 0.0
    ? (outputContrast * 100.0).toFixed(places)
    : polarity + "LOW";
} // Close SAPCcontrast()
...
function SAPCinverse(
  ...
  if (returnAs === "color") {
    let colorB = Math.min(255, Math.round(Math.pow(Ynew, mainTRCencode) * 255))
      .toString(16)
      .padStart(2, "0");
...
function RGBcolor(
  ...
  function parseColorString() {
    ...
    var HTMLcolors = {
      ...
      yellow: "ffff00",
      yellowgreen: "9acd32",
      controlgrey_1: "111111",
      controlgrey_2: "222222",
      controlgrey_3: "333333",
      controlgrey_4: "444444",
      controlgrey_5: "555555",
      controlgrey_6: "666666",
      controlgrey_7: "777777",
      controlgrey_8: "888888",
      controlgrey_9: "999999",
      controlgrey_a: "aaaaaa",
      controlgrey_b: "bbbbbb",
      controlgrey_c: "cccccc",
      controlgrey_d: "dddddd",
      controlgrey_e: "eeeeee"
    };
    ...
    var colorDefs = [
...

      {
        ...
        example: ["123,", "255,"],
        process: function(bits) {
          return [parseInt(bits[1]), parseInt(bits[1]), parseInt(bits[1])];
        }
      },
  ...
  //////////////////////////////////////////
  var RGBstringProperties = false; // reports if next section is available ////\ End of Switched Section /////
  //////////////////////////////////////////
  /* SECTION SWITCH  (remove/add first slash to toggle:  /* off   //* on )
...
//  */ //////////////////////////////////////////
  ...
  this.RGBmonoValueP =
    Math.min(Math.pow(this.LYP, mainTRCencode) * 255, 255) & 0xff;
  ...
  this.Rlin = function(exp = mainTRC) {
    return Math.pow(this.r / 255.0, exp);
  };
  this.Glin = function(exp = mainTRC) {
    return Math.pow(this.g / 255.0, exp);
  };
  this.Blin = function(exp = mainTRC) {
  ...
  this.YunClmp = function(YtoUnClamp = this.LYP) {
    YtoUnClamp =
      YtoUnClamp > blkThrs
        ? YtoUnClamp
        : YtoUnClamp < Math.pow(blkThrs, blkClmp)
        ? 0.0
  ...
  function sRGB8bit(linChan) {
    return Math.max(
      0,
      Math.min(255, Math.round(piecewiseEncode(linChan) * 255.0))
    );
  }
  ...
  this.sRlin = function(sChan = this.r) {
  ...
  const cubeRoot = 0.33333333333333333333; // Math.pow(n, cubeRoot) ////\ End of Switched Section /////
...
//  */ //////////////////////////////////////////
  ...
  function XYZtoLinRGB() {
    ...
    this.linR = this.X * 3.2404542 + this.Y * -1.5371385 + this.Z * -0.4985314;
  ...
  this.toY100clamped = function(places = 4) {
    ...
    return isNaN(clamped)
      ? clamped
      : "Yc" +
          (100 * (clamped + Math.pow(blkThrs - clamped, blkClmp)))
            .toPrecision(places)
            .substr(0, places + 4);
  };
      ...
      : [
          "Y " +
            (100 * (clamped + Math.pow(blkThrs - clamped, blkClmp)))
              ...
              .substr(0, places + 4),
          "Δ " +
            (100 * Math.pow(blkThrs - clamped, blkClmp))
              .toPrecision(places)
              .substr(0, places + 4),
          type,
          "ebu2"
